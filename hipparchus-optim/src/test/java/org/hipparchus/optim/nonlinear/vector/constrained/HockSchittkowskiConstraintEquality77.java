...
public class HockSchittkowskiConstraintEquality77 extends EqualityConstraint {
    ...
    public RealVector value(RealVector x) {
       ...
       //  x[1]^2*x[4] + sin(x[4]-x[5]) = 2*sqrt(2)
        ...
        a.setEntry(0,x1*x1*x4+Math.sin(x4-x5));
    ...
    public RealMatrix jacobian(RealVector x) {
      ...
      a.setEntry(0,1,0.0);
      a.setEntry(0,2,0.0);
      a.setEntry(0,3,x1*x1+Math.cos(x4-x5));
      a.setEntry(0,4,-1.0*Math.cos(x4-x5));
       a.setEntry(1,0,0.0);
        a.setEntry(1,1,1.0);
    ...
    {   RealVector lb=new ArrayRealVector(2);
        lb.setEntry(0,2.0*Math.sqrt(2.0));
        lb.setEntry(1,8.0+1.0*Math.sqrt(2.0));
