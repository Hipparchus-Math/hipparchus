 ...
 * @since 3.1
 */
public class SQPOptimizer2 extends ConstraintOptimizer {
...
//            }
          ...
          // if (currentPenaltyGrad < 0.0) {
            ...
            double currentPenalty = penaltyFunction(functionEval, x, y, r, y);
...
//                    y = y.add((dy.subtract(y)).mapMultiply(alfa));
           ...
           // if (search < maxLineSearchIteration) failedSearch = 0;
            ...
            double oldF = functionEval;
            functionEval = alfaF;
            RealVector oldGrad = functionGradient.copy();
...
//partial -= firstTerm.dotProduct(dx) - secondTerm.dotProduct(dx) + g.dotProduct(dye);
    ...
    private LagrangeSolution solveQP(RealVector x, RealVector y, double rho) {
        ...
        if (iqConstraint != null) {
            ...
            mi = iqConstraint.dimY();
            violated = (inequalityEval.subtract(iqConstraint.getLowerBound()).getMinValue() <= epsilon) || (y.getMinValue() > 0);
