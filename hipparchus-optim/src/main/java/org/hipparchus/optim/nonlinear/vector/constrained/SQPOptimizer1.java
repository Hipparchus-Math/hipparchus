 ...
 * @since 3.1
 */
public class SQPOptimizer1 extends ConstraintOptimizer {
    ...
    public LagrangeSolution doOptimize() {
        ...
        double maxGrad = this.obj.gradient(x).getLInfNorm();

        RealMatrix currentH = null;
        if (this.eqConstraint != null) {
            maxGrad = FastMath.max(maxGrad, this.eqConstraint.value(x).getLInfNorm());
        }
        if (this.iqConstraint != null) {
            maxGrad = FastMath.max(maxGrad, this.iqConstraint.value(x).getLInfNorm());
        }
        if (useFunHessian == false) {
            currentH = MatrixUtils.createRealIdentityMatrix(x.getDimension()).scalarMultiply(maxGrad);
        } else {
        ...
        for (int i = 0; i < this.getMaxIterations(); i++) {
            ...
            while (sigma > sigmaMax && qpLoop < this.qpMaxLoop) {
                sol1 = solveQP(currentH, currentGrad, x, y, rho);
                sigma = sol1.getValue();
...
//            if (convCriteria == 0) {
//                if (dx.dotProduct(currentH.operate(dx)) < epsilon * epsilon) {
//                    break;
//                }
//            } else {
//                if (dx.getNorm() < epsilon * (1 + x.getNorm())) {
//                    break;
//                }
//
//            }
          ...
          // if (currentPenaltyGrad < 0.0) {
            ...
            if (convCriteria == 0) {
                if (dx.mapMultiply(alfa).dotProduct(currentH.operate(dx.mapMultiply(alfa))) < epsilon * epsilon)
                {
...
//                    y = y.add((dy.subtract(y)).mapMultiply(alfa));
           ...
           if (failedSearch == 2) {
                ...
                failedSearch = 0;
//            EigenDecomposition dsX = new EigenDecomposition(currentH);
//            double min = new ArrayRealVector(dsX.getRealEigenvalues()).getMaxValue();
//           reset
//            currentH = MatrixUtils.createRealIdentityMatrix(dx.getDimension()).scalarMultiply(min);
                resetHessian = true;
            }
...
//partial -= firstTerm.dotProduct(dx) - secondTerm.dotProduct(dx) + g.dotProduct(dye);
    ...
    private double updateRho(RealVector dx, RealVector dy, RealMatrix H, RealMatrix jacobianG, double additionalVariable, double rho) {
        double num = 10.0 * FastMath.pow(dy.dotProduct(jacobianG.operate(dx)), 2);
    ...
    private LagrangeSolution solveQP(RealMatrix H, RealVector g, RealVector x, RealVector y, double rho) {
        ...
        if (iqConstraint != null) {
            ...
            mi = iqConstraint.dimY();
            violated = (iqConstraint.value(x).subtract(iqConstraint.getLowerBound()).getMinValue() <= epsilon) || (y.getMinValue() > 0);
