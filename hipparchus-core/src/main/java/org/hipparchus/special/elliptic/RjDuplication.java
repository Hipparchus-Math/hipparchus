 ...
 * @since 2.0
 */
class RjDuplication<T extends CalculusFieldElement<T>> extends Duplication<T> {
     ...
     */
    RjDuplication(final T x, final T y, final T z, final T p) {
        super(x, y, z, p);
    ...
    protected T evaluate(final T[] v0, final T a0, final T aM, final  double fourM) {
        ...
        final T rcTerm = new RcDuplication<>(a0.getField().getOne(), delta.divide(sM.multiply(sM).multiply(fourM)).add(1)).
                         integral().
