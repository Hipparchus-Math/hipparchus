...
public class LineTest {
    ...
    public void testContains() {
        ...
        Assertions.assertTrue(l.contains(new Vector2D(0, 1)));
        Assertions.assertTrue(l.contains(new Vector2D(1, 2)));
        Assertions.assertTrue(l.contains(new Vector2D(7, 8)));
        Assertions.assertFalse(l.contains(new Vector2D(8, 7)));
    }
    ...
    public void testOffset() {
        Line l = new Line(new Vector2D(2, 1), new Vector2D(-2, -2), 1.0e-10);
        Assertions.assertEquals(-5.0, l.getOffset(new Vector2D(5, -3)), 1.0e-10);
        Assertions.assertEquals(+5.0, l.getOffset(new Vector2D(-5, 2)), 1.0e-10);
    }

    @Test
    public void testDistance() {
        Line l = new Line(new Vector2D(2, 1), new Vector2D(-2, -2), 1.0e-10);
        Assertions.assertEquals(+5.0, l.distance(new Vector2D(5, -3)), 1.0e-10);
        Assertions.assertEquals(+5.0, l.distance(new Vector2D(-5, 2)), 1.0e-10);
    }
    ...
    public void testPointAt() {
        ...
        for (double a = -2.0; a < 2.0; a += 0.2) {
            ...
            Assertions.assertEquals(a, (l.toSubSpace(point)).getX(), 1.0e-10);
            Assertions.assertEquals(0.0, l.getOffset(point),   1.0e-10);
            for (double o = -2.0; o < 2.0; o += 0.2) {
                point = l.getPointAt((Vector1D) pA, o);
                Assertions.assertEquals(a, (l.toSubSpace(point)).getX(), 1.0e-10);
                Assertions.assertEquals(o, l.getOffset(point),   1.0e-10);
            }
        }
    }
    ...
    public void testOriginOffset() {
        ...
        Assertions.assertEquals(FastMath.sqrt(0.5), l1.getOriginOffset(), 1.0e-10);
        Line l2 = new Line(new Vector2D(1, 2), new Vector2D(0, 1), 1.0e-10);
        Assertions.assertEquals(-FastMath.sqrt(0.5), l2.getOriginOffset(), 1.0e-10);
    }
    ...
    public void testParallel() {
        ...
        Assertions.assertTrue(l1.isParallelTo(l2));
        ...
        Assertions.assertFalse(l1.isParallelTo(l4));
    }
    ...
    public void testTransform() throws MathIllegalArgumentException {
        ...
        Assertions.assertEquals(0.5 * FastMath.PI,
        ...
        Transform<Euclidean2D, Euclidean1D> t2 =
            Line.getTransform(0.0, 0.5, -1.0, 0.0, 1.0, 1.5);
        Assertions.assertEquals(FastMath.atan2(1.0, -2.0),
    ...
    public void testIntersection() {
        ...
        Assertions.assertEquals(0.5, p.getX(), 1.0e-10);
        Assertions.assertEquals(1.5, p.getY(), 1.0e-10);
    }
